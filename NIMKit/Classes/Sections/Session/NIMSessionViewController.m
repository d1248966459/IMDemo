...
- (void)viewDidLoad {
    ...
    if (self.session.sessionType == NIMSessionTypeTeam) {
        BOOL isIn =  [self checkInTeam];
        if (!isIn) {
            UIAlertView * alterView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"你已经不再群组内" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
            alterView.tag = 11111;
            [alterView show];
        }
    }
...
-(BOOL)checkInTeam{
    return [[NIMSDK sharedSDK].teamManager isMyTeam:self.session.sessionId];
}
...
-(BOOL)isMyComputer{
    ...
    if ([self.session.sessionId isEqualToString:me.userId]) {
        return YES;
    }
