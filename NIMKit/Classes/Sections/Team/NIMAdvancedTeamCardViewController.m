...
-(BOOL)isSystemGroup{
    NSData *jsonData = [self.team.serverCustomInfo dataUsingEncoding:NSUTF8StringEncoding];
    ...
    if (err) {
        ...
        return [[dic objectForKey:@"sys"] boolValue];
    }
}
...
- (void)layoutSubviews{
    ...
    if ([self isSystemGroup]) {
        self.titleLabel.nim_centerY = self.avatar.nim_centerY;
    }else{
